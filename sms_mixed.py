...
l = torch.abs(torch.tensor(np.load("Data/sms/train_L_S_smooth.npy")[:, 0]).long())
l_test = torch.abs(torch.tensor(np.load("Data/sms/test_L_S_smooth.npy")[:, 0]).long())

# Continuous score values
s = torch.tensor(np.load("Data/sms/train_L_S_smooth.npy")[:, 1]).double()
s_test = torch.tensor(np.load("Data/sms/test_L_S_smooth.npy")[:, 1]).double()

# Labeling Function Classes
k = torch.tensor(np.load("Data/sms/k.npy")).long()

# True y
y_true_test = np.load("Data/sms/true_labels_sms.npy")

continuous_mask = torch.ones(n_lfs).double()  # All labeling functions are continuous
...
optimizer = optim.Adam([theta, pi, pi_y], lr=0.01, weight_decay=0)
